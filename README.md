...
2026-09-21 已恢复暂停的 Supabase 服务，用户随后确认网站恢复。此前另一台设备的整站连接超时未在本机复现，不能将其原因直接归为 Supabase 暂停。3.4.0 已通过本地验收、GitHub 检查及 Vercel 部署；正式域名首页、录入和诊断页面已在浏览器验证，详情与边界见本轮更新记录。
