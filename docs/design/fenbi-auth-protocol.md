...
用户已用本人粉笔 App 扫码确认。云端 getIdentity 成功，官方会话 Cookie 作用域允许题库域，但首次题树返回 HTTP 453（设备登记要求），不能通过重复扫码解决。

后续实现采用真实网站浏览器按官网公开算法采集八项设备字段，由同一扫码会话调用官方设备创建接口一次，仅保留官方实际返回的 deviceId。正常登记已被接受，云端开始题库读取；最终题量和完整性由操作记录追加。不是把本机旧 Cookie/deviceSid 复制到服务器，也没有伪造浏览器字段或绕过人工验证。后端先以条件写标记本会话已尝试，断开/重连使旧登记结果失效；若接口仍要求验证则停止尝试。
